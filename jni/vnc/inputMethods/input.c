...
void initInput()
{
	...
	if((inputfd = suinput_kbd_open("Generic Keyboard", &id[0])) == -1) {
	...
	if((inputfd_touch = suinput_touch_open("Generic Touch", &id[1])) == -1) {
		L("cannot create virtual touch device.\n");
		sendMsgToGui("~SHOW|Cannot create virtual input device!\n");
		//  exit(EXIT_FAILURE); do not exit, so we still can see the framebuffer
	}
}
...
#define MOUSE_MIDDLE_KEY	0x02
#define MOUSE_RIGHT_KEY		0x04
#define MOUSE_WHEEL_FORWARD	0x08
#define MOUSE_WHEEL_BACK	0x10
...
void touchEvent(int buttonMask, int x, int y, rfbClientPtr cl)
{
	...
	if (buttonMask & MOUSE_LEFT_KEY) {
		suinput_write(inputfd_touch, EV_ABS, ABS_MT_TOUCH_MAJOR, 100);
		suinput_write(inputfd_touch, EV_ABS, ABS_MT_TRACKING_ID, 0);
		if (!(btn_pressed & MOUSE_LEFT_KEY))
			suinput_write(inputfd_touch, EV_KEY, BTN_TOUCH, 1);
		suinput_write(inputfd_touch, EV_ABS, ABS_MT_POSITION_X, x);
		suinput_write(inputfd_touch, EV_ABS, ABS_MT_POSITION_Y, y);
		suinput_write(inputfd_touch, EV_SYN, SYN_MT_REPORT, 0);
		btn_pressed |= MOUSE_LEFT_KEY;
	} else if (btn_pressed & MOUSE_LEFT_KEY) {
		btn_pressed &= ~MOUSE_LEFT_KEY;
		suinput_write(inputfd_touch, EV_ABS, ABS_MT_POSITION_X, x);
		suinput_write(inputfd_touch, EV_ABS, ABS_MT_POSITION_Y, y);
		suinput_write(inputfd_touch, EV_KEY, BTN_TOUCH, 0);
		suinput_write(inputfd_touch, EV_SYN, SYN_MT_REPORT, 0);
	}

	suinput_write(inputfd_touch, EV_SYN, SYN_REPORT, 0);
}
...
void mouseEvent(int buttonMask, int x, int y, rfbClientPtr cl)
{
	...
	//L("cursor udpate: mask %02x, x %d, y %d\n", buttonMask, x, y);
	if ( inputfd_touch == -1 )
		return;

	setIdle(0);
	//transformTouchCoordinates(&x,&y,cl->screen->width,cl->screen->height);
	//L("after cursor udpate: mask %02x, x %d, y %d\n", buttonMask, x, y);

	if (x != old_x) {
		ret = suinput_write(inputfd_touch, EV_REL, REL_X, x - old_x);
		if (!ret)
			old_x = x;
	}

	if (y != old_y) {
		ret = suinput_write(inputfd_touch, EV_REL, REL_Y, y - old_y);
		if (!ret)
			old_y = y;
	}
	...
	if (buttonMask & MOUSE_LEFT_KEY) {
		btn_pressed |= MOUSE_LEFT_KEY;
		suinput_write(inputfd_touch, EV_MSC, MSC_SCAN, 90001);
		suinput_write(inputfd_touch, EV_KEY, BTN_LEFT, 1);
	} else if (btn_pressed & MOUSE_LEFT_KEY) {
		btn_pressed &= ~MOUSE_LEFT_KEY;
		suinput_write(inputfd_touch, EV_MSC, MSC_SCAN, 90001);
		suinput_write(inputfd_touch, EV_KEY, BTN_LEFT, 0);
	}

	if (buttonMask & MOUSE_MIDDLE_KEY) {
		btn_pressed |= MOUSE_MIDDLE_KEY;
		suinput_write(inputfd_touch, EV_MSC, MSC_SCAN, 90003);
		suinput_write(inputfd_touch, EV_KEY, BTN_MIDDLE, 1);
	} else if (btn_pressed & MOUSE_MIDDLE_KEY) {
		btn_pressed &= ~MOUSE_MIDDLE_KEY;
		suinput_write(inputfd_touch, EV_MSC, MSC_SCAN, 90003);
		suinput_write(inputfd_touch, EV_KEY, BTN_MIDDLE, 0);
	}

	if (buttonMask & MOUSE_RIGHT_KEY) {
		btn_pressed |= MOUSE_RIGHT_KEY;
		suinput_write(inputfd_touch, EV_MSC, MSC_SCAN, 90002);
		suinput_write(inputfd_touch, EV_KEY, BTN_MIDDLE, 1);
	} else if (btn_pressed & MOUSE_RIGHT_KEY) {
		btn_pressed &= ~MOUSE_RIGHT_KEY;
		suinput_write(inputfd_touch, EV_MSC, MSC_SCAN, 90002);
		suinput_write(inputfd_touch, EV_KEY, BTN_MIDDLE, 0);
	}

	if (buttonMask & MOUSE_WHEEL_FORWARD) {
		suinput_write(inputfd_touch, EV_REL, REL_WHEEL, 1);
	}

	if (buttonMask & MOUSE_WHEEL_BACK) {
		suinput_write(inputfd_touch, EV_REL, REL_WHEEL, -1);
	}
